...
interface Props extends FormHTMLAttributes<HTMLFormElement> {
    className?: string,
    fetchViajes: (idCiudadSalida: number | string, idCiudadDestino: number | string, fechaIda: string) => void,
    setCiudadSalidaProp: React.Dispatch<React.SetStateAction<IType | null>>
    setCiudadDestinoProp: React.Dispatch<React.SetStateAction<IType | null>>
...
const FormInlineTemplateDependent = ({ className = '', fetchViajes, setCiudadSalidaProp, setCiudadDestinoProp, setFechaIdaProp, ...props }: Props) => {
    ...
    const enviar = (eve: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!Object.values(erros).some(error => !!error)) {
            ...
            fetchViajes(ciudadSalida!.value, ciudadDestino!.value, dataConvert(fechaIda!))
    ...
    const fetchCiudad = (idCiudad: number, setter: React.Dispatch<React.SetStateAction<IType | null>>, setterProp: undefined | React.Dispatch<React.SetStateAction<IType | null>>) => {
        http.get<ICiudad>(`ciudades/${idCiudad}`)
            .then(({ data }) => {
                const ciudadType = { label: capitalizeFirstLetter(data.nombre), value: data.id }
                setter(ciudadType)
                if (setterProp) setterProp(ciudadType)
            });
    };
    useEffect(() => {
